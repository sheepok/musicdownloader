...
class Credential:
    ...
    def asjson(self) -> str:
        ...
        return _fast_dumps(data).decode()
    '''fromcookiesdict'''
    @classmethod
...
class QQMusicClientUtils(object):
    ...
    def sign(request: dict) -> str:
        ...
        hash = hashlib.sha1(_fast_dumps(request)).hexdigest().upper()
    ...
    def randompayloadbydevice(device: Device, version: str):
        ...
        return {
            ...
            "osVersion": f"Android {device.version.release},level {device.version.sdk}", "qimei": "", "qimei36": "", "sdkVersion": "1.2.13.6", "targetSdkVersion": "33", "audit": "", "userId": "{}", "packageId": "com.tencent.qqmusic", "deviceType": "Phone", "sdkName": "", "reserved": _fast_dumps(reserved).decode(),
        }
    '''obtainqimei'''
    @staticmethod
    def obtainqimei(version: str, device: Device) -> QimeiResult:
        try:
            ...
            params = base64.b64encode(QQMusicClientUtils.aesencrypt(crypt_key.encode(), _fast_dumps(payload))).decode()
            ...
            device.qimei = (data := _fast_loads(_fast_loads(resp.content)["data"])["data"])["q36"]
